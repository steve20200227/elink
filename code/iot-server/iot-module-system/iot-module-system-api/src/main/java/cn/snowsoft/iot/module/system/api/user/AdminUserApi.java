...
import cn.snowsoft.iot.module.system.enums.ApiConstants;
...
public interface AdminUserApi extends AutoTransable<AdminUserRespDTO> {
    ...
    CommonResult listByRole(@RequestParam("id") String id);

    @GetMapping("list-by-role-dept")
    CommonResult<List<AdminUserRespDTO>> listByRoleAndDept(@RequestParam("deptIds") Collection<Long> deptIds,@RequestParam("roleIds") Collection<Long> roleIds);

    @GetMapping("list-user-by-role")
    CommonResult<List<AdminUserRespDTO>> listUserByRole(@RequestParam("id") String id);
}
